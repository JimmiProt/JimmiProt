...
export default function Scoreboard({onBack}){
 ...
 const {enterFullscreen,exitFullscreen}=useFullscreenWakeLock(root);
 ...
 async function fullscreen(){const result=await enterFullscreen();if(!result.fullscreen){setNotice('Fullscreen is unavailable in this browser. The board still fills the page; rotate your phone for landscape.');return;}try{await screen.orientation?.lock?.('landscape');}catch{}setNotice(result.wakeLock?'':'Fullscreen is active. Screen-awake control is unavailable in this browser or device mode.');}

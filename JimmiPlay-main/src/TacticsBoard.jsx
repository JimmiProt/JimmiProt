...
import {ArrowLeft, Maximize, Minimize, RotateCcw, Users, User} from 'lucide-react';
import {initialPieces,movePiece} from './tactics';
import useFullscreenWakeLock from './useFullscreenWakeLock';
import './tactics.css';
...
export default function TacticsBoard({onBack}) {
  ...
  const root=useRef(null), board=useRef(null), drag=useRef(null);
  const {isFullscreen,enterFullscreen,exitFullscreen}=useFullscreenWakeLock(root);
  function start(e,p) {
  ...
  async function toggleFullscreen(){
    ...
    if(!result.fullscreen)setNotice('Fullscreen is unavailable in this browser. You can still use the board on this page.');
    else if(!result.wakeLock)setNotice('Fullscreen is active. Screen-awake control is unavailable in this browser or device mode.');
  }
  async function leave(){await exitFullscreen();onBack();}
  return <main className="tactics-page" ref={root}><header className="tactics-heading"><button onClick={leave} aria-label="Back to tools"><ArrowLeft size={20}/></button><h1>Tactics Board</h1><div className="tactics-heading-actions"><button onClick={()=>setPieces(initialPieces(mode))} aria-label="Reset positions"><RotateCcw size={19}/></button><button onClick={toggleFullscreen} aria-label={isFullscreen?'Exit fullscreen':'Enter fullscreen'}>{isFullscreen?<Minimize size={19}/>:<Maximize size={19}/>}</button></div></header><p className="tactics-hint">Drag players and the shuttle to plan your next rally.</p>
